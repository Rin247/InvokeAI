...
<Steps>
  ...
  2. Create a directory for your installation, typically in your home directory (e.g. `~/invokeai` or `$Home/invokeai`):
   ...
   3. Create a virtual environment in that directory:
       ...
       ```sh
       uv venv --relocatable --prompt invoke --python 3.13 --python-preference only-managed .venv
       ```

       This command creates a portable virtual environment at `.venv` complete with a portable python 3.13. It doesn't matter if your system has no python installed, or has a different version - `uv` will handle everything.
          ...
          <Tabs syncKey="unixGpuType">
            ...
            <TabItem label="Linux + AMD GPU">
              ...
              :::caution[ROCm and torch 2.12]
                ...
                ```sh
                 uv pip install <PACKAGE_SPECIFIER>==<VERSION> "torch<2.12" --python 3.13 --python-preference only-managed --torch-backend=rocm7.1 --force-reinstall
                ```
  ...
  8. Install the `invokeai` package. Substitute the package specifier and version.
      ...
      <Tabs synckKey="torchBackend">
        <TabItem label="No torch backend">
          ```sh
          uv pip install <PACKAGE_SPECIFIER>==<VERSION> --python 3.13 --python-preference only-managed --force-reinstall
          ```
        </TabItem>
        <TabItem label="With torch backend">
          ```sh
          uv pip install <PACKAGE_SPECIFIER>==<VERSION> --python 3.13 --python-preference only-managed --torch-backend=<VERSION> --force-reinstall
          ```
        </TabItem>
