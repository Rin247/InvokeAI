...
Invoke runs on Windows 11+ and Linux (Ubuntu 24.04+ and Arch Linux rolling are well-tested).
...
Invoke requires Python 3.13. Python 3.14 and 3.15 are work-in-progress targets pending the Qt6 frontend refactor; they may work, but are not yet fully validated.

Check that your system has an up-to-date Python installed by running `python3 --version` in the terminal (Linux) or cmd/powershell (Windows).
  ...
  <Tabs syncKey="operatingSystem">
    ...
    <TabItem label="Linux" icon="linux">
      <Steps>
        ...
        2. You'll need to install `libglib2.0-0` and `libgl1-mesa-glx` for OpenCV to work. For example, on a Debian/Ubuntu system: `sudo apt update && sudo apt install -y libglib2.0-0 libgl1-mesa-glx`
      </Steps>
    </TabItem>
  </Tabs>
:::
