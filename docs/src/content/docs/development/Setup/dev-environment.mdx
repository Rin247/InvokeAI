...
<Steps>
  ...
  2. Fork and clone the InvokeAI git repository.
   ...
   3. Git LFS is not installed in this development environment, and the pre-push LFS hook has been removed. If you need LFS assets, install git-lfs manually and run `git lfs pull` after cloning.

       :::warning[Remote push policy]
         Some clones of this repo carry additional remotes such as `upstream` or `krakotay`.
         **These remotes are read-only.** You may pull from them, but you must never push to
         them directly. All pushes must go to your own fork (`origin` or equivalent).
       :::
  ...
  5. Follow the [manual install](/start-here/manual/) guide, with some modifications to the install command:
      ...
      With the modifications made, the sync command should look something like this:
       ...
       :::note[Python versions]
         Python 3.13 is the current working local development target. Python 3.14 and 3.15 are planned but currently WIP due to missing dependency wheels.
       :::
