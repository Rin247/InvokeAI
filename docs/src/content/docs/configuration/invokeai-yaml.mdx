...
- On a system without a CUDA GPU, `auto` resolves to the single best available device (`cpu`), so generation runs serially.
...
- `syslog` is only available on Linux systems. It uses
