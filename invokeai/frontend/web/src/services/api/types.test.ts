...
const partialConfig = (base: 'z-image', submodels: Record<string, unknown>) => ({
  type: 'main',
  base,
...
// The Anima loader accepts a 16-channel Wan VAE beside the Anima/QwenImage one, but Krea-2
// draws its own VAE pool from the base-driven `isAnimaVAEModelConfig` and must not be offered Wan VAEs.
...
describe('Anima VAE predicates', () => {
  ...
  it.each(['qwen-image'])('rejects a %s VAE in both predicates', (base) => {
  ...
  describe('main-model VAE submodels', () => {
    ...
    it('changes its verdict based on the second argument', () => {
      const config = mainWithVae('anima');
